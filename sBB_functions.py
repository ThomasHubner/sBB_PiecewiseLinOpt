...
def getPLFvalue(breakpoints_x, breakpoints_y, point):
    '''
    Returns the value of the PLF defined by 
    "breakpoints_x" (list) and "breakpoints_y" (list)
    at the point "point"(float).

    '''
    
    if point <= breakpoints_x[0]:
        return breakpoints_y[0]
...
def getEnvelope(breakpoints_x, breakpoints_y, interval):
    '''
...
def solve_node(side, m, var, con, branch_index, branch_point,
             ...
             pos, env_breakpoints_x, env_breakpoints_y):
    '''
    Solves a child node in the branch-and-bound.
    ...
    new_lower_x, new_lower_y, env_runtime = getEnvelope(
        ...
        PLFs_breakpoints_y[branch_index], interval)
    ...
    plf_values = [getPLFvalue(PLFs_breakpoints_x[i], PLFs_breakpoints_y[i], m.x[i]) 
               for i in range(0, n)]
